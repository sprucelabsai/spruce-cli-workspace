...
class SpruceModuleLoader {
	...
	public compilerOptions: Record<string, any> = require('../../tsconfig.json').compilerOptions
	...
	public constructor() {
		...
		Object.keys(this.compilerOptions.paths).forEach(alias => {
			this.replacePaths[alias.replace(/\*.?/, '(.*)')] = this.compilerOptions.paths[
				alias
			].map((destination: string) => destination.replace(/\*.?/, '$1'))
		})

		// store original resolveFilename from core module loader to be called after the we do the mapping below
		//@ts-ignore
		coreModuleLoader._originalResolveFilename = coreModuleLoader._resolveFilename
		...
		coreModuleLoader._resolveFilename = (
			...
			// @ts-ignore
			return coreModuleLoader._originalResolveFilename(
				resolved,
				parent,
				isMain
			)
